...
def yahoo_stock_query(ticker,
    ...
    # example url for financials:
    # balance_sheetq='balanceSheetHistoryQuarterly'
    #financial_url=host+'/v10/finance/quoteSummary/'+ticker+'?modules='+balance_sheetq
    #financial_json=pd.read_json(financial_url)
    # useful, example result from the json:
    # financial_json['quoteSummary']['result'][0]['balanceSheetHistoryQuarterly']['balanceSheetStatements']
    
    # example url for revenue:
    # an interesting ticker: 'ADRNY'
    #income_statementq='incomeStatementHistoryQuarterly'
    # alternative, for annual?
    #income_statementq='incomeStatementHistory'
    #income_url=host+'/v10/finance/quoteSummary/'+ticker+'?modules='+income_statementq
    #income_json=pd.read_json(income_url)
    # useful example result from the json, for what appears to be the earliest
    # statement:
    #date=pd.to_datetime(income_json['quoteSummary']['result'][0][income_statementq]['incomeStatementHistory'][-1]['endDate']['fmt'])
    #rev=income_json['quoteSummary']['result'][0][income_statementq]['incomeStatementHistory'][-1]['totalRevenue']['raw']
